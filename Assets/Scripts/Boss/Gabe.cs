...
public class Gabe : MonoBehaviour {
    ...
    IEnumerator Fire()
    {
      while (_hp >= 0)
      {
       if (!_specSkill)
       {
          ...
          Instantiate(_bullet[Random.Range(0, _bullet.Length)], transform.position, Quaternion.Euler(0, 0, rot));
    ...
    IEnumerator SpecSkill()
    {
        ...
        {
            Instantiate(_bullet[Random.Range(0, _bullet.Length)], transform.position, Quaternion.Euler(0, 0, i));
        }
        yield return new WaitForSeconds(1f);
        for (int i = -50; i < 50; i += 5)
        {
